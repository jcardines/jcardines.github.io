...
df %>% 
  ...
  ggplot(aes(y=Cumulative_deaths, x=Date_reported, color=Country_code)) + geom_line() + transition_reveal(Date_reported)
```
...
```{r}
 df %>% 
  filter(Country_code %in% c('US', 'IT', 'MX')) %>%
   ggplot(aes(y=death_per_cases, x=Date_reported, color=Country_code)) + geom_line() + transition_reveal(Date_reported)
```
```
```
