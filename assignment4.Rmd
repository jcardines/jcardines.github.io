...
```{r}
df$weekend <- case_when(df$weekday >=6 ~ '1', TRUE ~ '0')
```
...
df %>% 
  ...
  group_by(month) %>% 
  summarise(medpositive_increase = median(positiveIncrease))
```
...
df %>%
  ...
  group_by(weekend) %>%
  summarise(avghospitalizations = mean(hospitalizedIncrease))
```
...
df %>% filter(date >= '2021-01-01', date <'2022-01-01') %>%
  group_by(weekday) %>% 
  summarise(median_cases = median(cases))

df$month = month(df$date)
df %>%  filter(month >= 6, month <10) %>%
  group_by(month) %>% 
  summarise(median_cases = median(cases)) 
...
- mutate
```{r}
df %>%  mutate(price_in_thousands = Price*1000)
df <- df %>%  mutate(price_in_thousands = Price*1000)
```

- summarise
```{r}
df %>% summarise(mean_price = mean(price_in_thousands))
```

- arrange
```{r}
df %>%  arrange(-price_in_thousands)
```
...
- count + arrange
```{r}
df %>% count(Model) %>% arrange(-n)
```

- filter + count + arrange
```{r}
df %>% 
  filter(price_in_thousands >- 15000) %>% 
...
df %>% 
  group_by(Year) %>% 
  summarise(median_price = median(price_in_thousands))
```
...
df %>% 
  ...
  group_by(Seller) %>% 
summarise(mean_price = mean(price_in_thousands))
```

- filter + group_by + summarise + arrange
```{r}
df %>% 
  filter(Seller =="Dealer", Year >= 2015) %>% 
  group_by(Model) %>% 
  summarise(mean_price = mean(price_in_thousands)) %>% 
  arrange(-mean_price)
```
