...
```{r}
df %>% 
  filter(location=="Argentina", date >'2021-12-31')%>% 
  ggplot()+
...
```{r}
df %>% 
  filter(location %in% c('Cyprus', 'Japan', 'Nepal', 'South Korea')) %>% 
  group_by(date, location) %>% 
  summarise(totalvaccines =sum(total_vaccinations, na.rm = TRUE)) %>% 
  ggplot()+
  geom_point(mapping=aes(x=date, y=totalvaccines, color = location))
```
...
```{r}
df %>% 
  filter(date>='2021-01-01', date < '2022-01-01', location %in% c('Chile', 'Ecuador')) %>% 
  group_by(date, location) %>% 
  summarise(totalvaccines =sum(total_vaccinations, na.rm = TRUE)) %>% 
  ggplot()+
  geom_point(mapping=aes(x=date, y=totalvaccines, color = location))
